...
Each plugin has its own file in `lua/tknvivid/groups/` that defines the highlight groups for that plugin. A typical plugin module looks like this:
...
For the complete list of available colors, see [colors.lua](lua/tknvivid/colors.lua).
...
2. **Create the plugin file**: Create a new file `lua/tknvivid/groups/plugin-name.lua`:
...
3. **Register the plugin**: Add an entry to the `M.plugins` table in `lua/tknvivid/groups/init.lua`:
...
- Look at similar highlight groups in [base.lua](lua/tknvivid/groups/base.lua) for consistency
...
vim.cmd("colorscheme tknvivid-storm")
vim.cmd("colorscheme tknvivid-moon")
...
1. Create `lua/tknvivid/groups/cool-finder.lua`:
...
2. Add to `lua/tknvivid/groups/init.lua`:
...
3. **Use template variables**: Reference colors using `${color_name}` syntax within the template string. Common colors include:
   ...
   - `${_name}` - the base name (e.g., "tknvivid_storm")
...
5. **Follow existing patterns**: Look at other extras like [kitty.lua](lua/tknvivid/extra/kitty.lua) or [alacritty.lua](lua/tknvivid/extra/alacritty.lua) for examples.

### How to Add a New Extra

1. Create a file like `lua/tknvivid/extra/cool-app.lua` following the template structure above.

2. Add the name and output file extension to the `extras` table in `lua/tknvivid/extra/init.lua`:
...
3. Run the build script from the tknvivid plugin directory:
