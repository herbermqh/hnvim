...
local Groups = require("tknvivid.groups")
...
function M.extras()
  local Extra = require("tknvivid.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/tknvivid/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/tknvivid/config.lua", "\n(--@class tknvivid%.Config.-\n})")
